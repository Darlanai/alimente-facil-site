...
const API_BASE_URL = 'http://localhost:3000';  // ou a porta onde seu back-end está rodando
...
this.elements.contactForm?.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    this.showNotification(data.message || 'Sua mensagem foi enviada com sucesso.', 'success');
    this.elements.contactForm.reset();
  } catch (error) {
    this.showNotification(error.message || 'Falha ao enviar mensagem.', 'error');
  } finally {
    if (submitBtn) {
...
/* === AUTH ROOT FIX | 2026-04-14 === */
(() => {
  const AUTH_TOKEN_KEY = 'alimenteFacilAuthToken';
  const AUTH_USER_KEY = 'alimenteFacilAuthUser';
  const APP_STATE_KEY = 'alimenteFacilState_vFinal';
  const PREMIUM_PLAN = 'premium';
  const BASIC_PLAN = 'basic';
  ...
  const resolveApiBase = () => {
    const explicit = [window.API_BASE_URL, window.__API_BASE_URL__].find((value) => typeof value === 'string' && value.trim());
  ...
  const readToken = () => {
    try { return localStorage.getItem(AUTH_TOKEN_KEY) || ''; } catch (_error) { return ''; }
  };
  ...
  const showAuthMessage = (formId, message, kind = 'error') => {
    const form = document.getElementById(formId);
    if (!form) return;
    const box = ensureFeedbackNode(form);
    if (!box) return;
    box.textContent = String(message || '');
    box.dataset.kind = kind;
    box.style.display = message ? 'block' : 'none';
    box.style.background = kind === 'success' ? 'rgba(52,199,89,.14)' : kind === 'info' ? 'rgba(0,242,234,.12)' : 'rgba(255,59,48,.12)';
    box.style.borderColor = kind === 'success' ? 'rgba(52,199,89,.35)' : kind === 'info' ? 'rgba(0,242,234,.35)' : 'rgba(255,59,48,.35)';
  };

  const clearAuthMessages = () => {
    document.querySelectorAll?.('.auth-feedback-message')?.forEach((node) => {
      node.textContent = '';
  ...
  const clearLegacyPanelFlags = (app) => {
    try {
  ...
  const closeAuthModal = (app) => {
    if (typeof app?.closeModal === 'function') {
      app.closeModal('auth-modal');
      return;
    }
    document.getElementById('auth-modal')?.classList?.remove('is-visible');
  };

  const install = () => {
    const app = window.app;
    if (!app) return false;
    if (app.__authRootFixInstalled === '1') return true;
    app.__authRootFixInstalled = '1';
    ...
    app.apiFetchJson = async function(url, options = {}) {
      const fullUrl = /^https?:/i.test(url) ? url : `${resolveApiBase()}${url}`;
      const headers = Object.assign({ 'Content-Type': 'application/json' }, options.headers || {});
      const response = await fetch(fullUrl, Object.assign({}, options, { headers }));
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
    ...
    app.getStoredAuthToken = readToken;
    app.setStoredAuthSession = writeSession;
    app.clearStoredAuthSession = clearSession;
    app.isPremiumSession = isPremiumPayload;
    ...
    app.showPaymentGateModal = function(payload = {}) {
      this.closePaymentGateModal?.();
      const checkoutUrl = getCheckoutUrl(this, payload);
      const overlay = document.createElement('div');
      overlay.id = 'payment-gate-modal';
      overlay.className = 'modal-overlay is-visible';
      overlay.innerHTML = `
        <div class="modal-box" style="max-width:520px; width:min(92vw,520px);">
          <button type="button" class="close-modal-btn" data-action="close-payment-gate" aria-label="Fechar">×</button>
          <div class="modal-header"><h3 style="margin:0;">${payload.title || 'Ative seu Premium'}</h3></div>
          <div class="modal-body" style="display:flex; flex-direction:column; gap:14px;">
            <p style="margin:0; color:var(--glass-text-primary); line-height:1.55;">${payload.message || 'Ative agora seu Premium com 7 dias grátis. Depois, R$ 9,90 por mês. Cancele quando quiser.'}</p>
            <div style="display:grid; gap:8px; padding:12px; border:1px solid rgba(255,255,255,.12); border-radius:16px; background:rgba(255,255,255,.04);">
              <div style="font-weight:700; color:#fff;">7 dias grátis</div>
      ...
      overlay.addEventListener('click', (event) => {
        if (event.target === overlay || event.target.closest?.('[data-action="close-payment-gate"]')) {
          overlay.remove?.();
          return;
        }
        if (event.target.closest?.('[data-action="go-checkout"]')) {
          if (!checkoutUrl) {
            this.showNotification?.('Link do Mercado Pago não encontrado.', 'error');
            return;
          }
    ...
    app.applyAuthenticatedUser = function(payload = {}) {
      const user = payload?.user || readStoredUser() || {};
      this.isLoggedIn = true;
      this.userPlan = isPremiumPayload(payload) ? PREMIUM_PLAN : BASIC_PLAN;
      this.state = this.state || deepClone(this.defaultState || {});
      this.state.user = this.state.user || {};
      this.state.user.nome = user.name || user.nome || this.state.user.nome || 'Usuário';
      this.state.user.email = user.email || this.state.user.email || '';
      this.state.user.id = user.id || this.state.user.id || '';
      this.updateStartButton?.();
      this.saveState?.();
    };
    ...
    app.syncRealUserInfoInDOM = function() {
      ...
      const configEmailModal = document.getElementById('config-email-modal');
      const configName = document.getElementById('config-name');
      const configNameModal = document.getElementById('config-name-modal');
      if (configEmail && email) configEmail.value = email;
      if (configEmailModal && email) configEmailModal.value = email;
      if (configName && name) configName.value = name;
      if (configNameModal && name) configNameModal.value = name;
    };

    const afterAuthSuccess = async (payload = {}, successMessage = '') => {
      clearAuthMessages();
      writeSession(payload?.token || '', payload?.user || null);
      app.applyAuthenticatedUser?.(payload);
      closeAuthModal(app);
      if (isPremiumPayload(payload)) {
        originalEnterAppMode ? originalEnterAppMode() : app.enterAppMode?.();
      } else {
        if (originalExitAppMode) originalExitAppMode();
        app.showPaymentGateModal?.({
          title: 'Ative seu Premium',
          message: successMessage || payload?.message || 'Conta criada com sucesso. Para liberar o painel completo, ative agora seu Premium com 7 dias grátis.',
          checkoutUrl: getCheckoutUrl(app, payload)
        });
      }
      setTimeout(() => app.syncRealUserInfoInDOM?.(), 50);
    };

    app.handleLogin = function() {
      return this._realHandleLogin?.();
    };

    app.handleSignup = function() {
      return this._realHandleSignup?.();
    };

    app._realHandleLogin = async function() {
      clearAuthMessages();
      const email = String(document.getElementById('login-email')?.value || '').trim();
      const password = String(document.getElementById('login-password')?.value || '');

      if (!email || !password) {
        showAuthMessage('login-form', 'Preencha e-mail e senha.');
        return false;
      }

      showAuthMessage('login-form', 'Entrando...', 'info');

      try {
        const payload = await this.apiFetchJson('/api/auth/login', {
          method: 'POST',
          body: JSON.stringify({ email, password })
        });
        await afterAuthSuccess(payload, payload?.message || 'Login realizado com sucesso.');
        return true;
      } catch (error) {
        showAuthMessage('login-form', error?.payload?.message || error.message || 'Não foi possível fazer login agora.');
        return false;
      }
    };

    app._realHandleSignup = async function() {
      clearAuthMessages();
      const name = String(document.getElementById('signup-name')?.value || '').trim();
      const email = String(document.getElementById('signup-email')?.value || '').trim();
      const password = String(document.getElementById('signup-password')?.value || '');
      ...
      if (!name || !email || !password) {
        showAuthMessage('signup-form', 'Preencha nome, e-mail e senha.');
        return false;
      }

      if (!acceptedTerms) {
        showAuthMessage('signup-form', 'Você precisa aceitar os Termos de Uso e a Política de Privacidade.');
        return false;
      }

      showAuthMessage('signup-form', 'Criando sua conta...', 'info');

      try {
        const payload = await this.apiFetchJson('/api/auth/register', {
          method: 'POST',
          body: JSON.stringify({ name, email, password, acceptedTerms })
        });
        await afterAuthSuccess(payload, payload?.message || 'Cadastro realizado com sucesso.');
        return true;
      } catch (error) {
        showAuthMessage('signup-form', error?.payload?.message || error.message || 'Não foi possível concluir seu cadastro agora.');
        return false;
      }
    };
    ...
    app.handleStartButtonClick = async function() {
      ...
      try {
        const payload = await this.apiFetchJson('/api/auth/me', {
          headers: { Authorization: `Bearer ${token}` }
        });
        writeSession(token, payload?.user || null);
        this.applyAuthenticatedUser?.(payload);
        if (isPremiumPayload(payload)) {
          originalEnterAppMode ? originalEnterAppMode() : this.enterAppMode?.();
        } else {
          if (originalExitAppMode) originalExitAppMode();
          this.showPaymentGateModal?.({ checkoutUrl: getCheckoutUrl(this, payload) });
        }
        setTimeout(() => this.syncRealUserInfoInDOM?.(), 50);
      } catch (_error) {
        this.forceLoggedOutLanding?.();
        this.showAuthModal?.();
      }
    ...
    app.handleLogout = function(...args) {
      clearSession();
      app.closePaymentGateModal?.();
      if (originalHandleLogout) return originalHandleLogout(...args);
      return app.forceLoggedOutLanding?.();
    };

    const bindAuthForm = (formId, handlerName) => {
      const form = document.getElementById(formId);
      if (!form || form.dataset.authRootFixBound === '1') return;
      form.dataset.authRootFixBound = '1';
      ensureFeedbackNode(form);
      form.addEventListener('submit', async (event) => {
        event.preventDefault?.();
        event.stopPropagation?.();
        event.stopImmediatePropagation?.();
        await app[handlerName]?.();
      }, true);
    };

    bindAuthForm('login-form', '_realHandleLogin');
    bindAuthForm('signup-form', '_realHandleSignup');
